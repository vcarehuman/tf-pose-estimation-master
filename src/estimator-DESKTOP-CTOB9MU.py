    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                import csv
                returns_path = "angle.xls"
                file = open(returns_path, "w")
                writer = csv.writer(file)
                writer.writerow(["CoordinateX", "CoordinateY"])
                #writer.writerow(center)  
                    
                    
                #import xlwt
                #wb = xlwt.Workbook()
                #ws = wb.add_sheet("angle.xls")
                #ws.write(0, 0, 'Coord_X')
                #ws.write(0, 1, 'Coord_Y')
                #ws.write(1, 0, int(body_part.x * image_w + 0.5))
                #ws.write(1, 1, int(body_part.y * image_w + 0.5))
                #wb.save("angle.xls")
